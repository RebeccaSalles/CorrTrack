...
class BalancedIndex:
    ...
    def find_pairs_with_dist(self, recent_entry_ids, win_sid_idx, win_time, tau):
        tau = float(tau)
        if tau < 0.0 or not self._entries:
            return []
        entries = self._entries
        ...
        for raw_id in np.asarray(recent_entry_ids, dtype=np.int64):
            ...
            left = bisect_left(entries, (lower, self._LOW_SENTINEL, self._LOW_SENTINEL))
            right = bisect_right(entries, (upper, self._HIGH_SENTINEL, self._HIGH_SENTINEL))
            for other_value, other_entry_id, other_idx in entries[left:right]:
                if other_entry_id == entry_id:
                    continue
                if other_idx < 0 or other_idx >= sid_idx.shape[0]:
                    continue
                ...
                pairs.append((int(ridx), int(other_idx), diff * diff))
        return pairs
    ...
    def find_pairs_full_with_dist(self, recent_entry_ids, win_sid_idx, win_time, tau):
        tau = float(tau)
        if tau < 0.0 or not self._entries or self.n_vectors <= 0:
            return []
        tau_sq = tau * tau
        ...
        for raw_id in np.asarray(recent_entry_ids, dtype=np.int64):
            ...
            for _v, other_entry_id, other_idx in entries[left:right]:
                ...
                if dist_sq > tau_sq:
                    continue
                pairs.append((int(ridx), int(other_idx), dist_sq))
        return pairs

    def find_pairs_full_parallel(self, recent_entry_ids, win_sid_idx, win_time, tau, n_workers=1):
        return self.find_pairs_full(recent_entry_ids, win_sid_idx, win_time, tau)

    def find_pairs_full_with_dist_parallel(self, recent_entry_ids, win_sid_idx, win_time, tau, n_workers=1):
        return self.find_pairs_full_with_dist(recent_entry_ids, win_sid_idx, win_time, tau)
...
def find_candidate_pairs_with_dist(values, value_window_idx, recent_values, recent_window_idx, win_sid_idx, win_time, tau):
    pairs = []
    values = np.asarray(values, dtype=np.float64).tolist()
    value_window_idx = np.asarray(value_window_idx, dtype=np.int64).tolist()
    recent_values = np.asarray(recent_values, dtype=np.float64).tolist()
    ...
    for val, ridx in zip(recent_values, recent_window_idx):
        ...
        for j in range(left, right):
            ...
            if win_sid_idx[other_idx] == sid_r and win_time[other_idx] == time_r:
                continue
            diff = float(val) - float(values[j])
            pairs.append((ridx, other_idx, diff * diff))
    return pairs
...
def find_candidate_pairs_full_with_dist(
    values,
    value_window_idx,
    recent_values,
    recent_window_idx,
...
def find_candidate_pairs_full_parallel(
    values,
    value_window_idx,
    recent_values,
    recent_window_idx,
    ...
    return find_candidate_pairs_full(
        values,
        value_window_idx,
        recent_values,
        recent_window_idx,
...
def find_candidate_pairs_full_with_dist_parallel(
    values,
    value_window_idx,
    recent_values,
    recent_window_idx,
    ...
    return find_candidate_pairs_full_with_dist(
        values,
        value_window_idx,
        recent_values,
        recent_window_idx,
