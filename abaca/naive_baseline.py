...
threshold, on the same span, m swept:
  ...
  bf_incremental      this project's incremental five-sum arm (the stricter baseline)
...
def naive_python(X, W, step, T, neg, n_lags=0):
    """Per pair, per step, per lag: np.corrcoef on the two windows. Counts pair-windows at or above T.

    (2026-09-23) `n_lags`: the same lagged universe the harness arms enumerate -- the current window of
    one series against the window `lag` earlier of the other, every ordered pair for lag > 0 and the
    upper triangle at lag 0 -- so the tiers stay comparable when lags are on."""
    m, N = X.shape
    count = 0
    t0 = time.perf_counter()
    for s0 in range(n_lags, N - W + 1, step):
        win = X[:, s0:s0 + W]
        for lag in range(0, n_lags + 1, step):
            hist = win if lag == 0 else X[:, s0 - lag:s0 - lag + W]
            for i in range(m):
                xi = win[i]
                for j in (range(i + 1, m) if lag == 0 else range(m)):
                    c = np.corrcoef(xi, hist[j])[0, 1]
                    if (abs(c) if neg else c) >= T:
                        count += 1
    return count, time.perf_counter() - t0


def naive_numpy(X, W, step, T, neg, n_lags=0):
    """Per step (and per lag): one vectorized correlation matrix over all series, BLAS all the way.

    COUNTS ONLY -- it never emits which pairs are correlated, applies none of the harness's
    near-constant / spike guards and keeps no state, which is why it is a reference point for the
    arithmetic tier and not a usable baseline arm (see the module docstring)."""
    m, N = X.shape
    count = 0
    t0 = time.perf_counter()
    ...
    for s0 in range(n_lags, N - W + 1, step):
        cur = X[:, s0:s0 + W]
        cz = cur - cur.mean(axis=1, keepdims=True)
        cz /= np.sqrt(np.einsum("ij,ij->i", cz, cz))[:, None]
        for lag in range(0, n_lags + 1, step):
            if lag == 0:
                C = (cz @ cz.T)[iu]
            else:
                hist = X[:, s0 - lag:s0 - lag + W]
                hz = hist - hist.mean(axis=1, keepdims=True)
                hz /= np.sqrt(np.einsum("ij,ij->i", hz, hz))[:, None]
                C = (cz @ hz.T).ravel()
...
def main() -> None:
    ...
    ap.add_argument("--n-lags", type=int, default=0, help="(2026-09-23) lagged universe; the incremental arm's advantage grows with it")
    ...
    for m in [int(v) for v in args.ms.split(",")]:
        ...
        base = dict(window_size=W, window_step=step, basic_window=step, n_lags=args.n_lags, corr_threshold=T, neg_corr=neg, exec="sequential",
        ...
        with tempfile.TemporaryDirectory() as tmp:
            for mode in ("bruteforce", "bf_incremental"):
                t0 = time.perf_counter()
        ...
        cnt, wall = naive_numpy(X, W, step, T, neg, args.n_lags); r["naive_numpy"] = {"wall": wall, "correlated": cnt}
        if m <= args.skip_python_above:
            cnt, wall = naive_python(X, W, step, T, neg, args.n_lags); r["naive_python"] = {"wall": wall, "correlated": cnt}
        bfw = r["bruteforce"]["wall"]
        # (2026-09-23, user) per pair-window cost: the wall divided by the pair-windows each arm computes
        # (m(m-1)/2 pairs x the number of steps), so the tiers can be compared independently of m and span
        n_steps = len(range(args.n_lags, X.shape[1] - W + 1, step))
        n_lag_probes = len(range(0, args.n_lags + 1, step))
        # lag 0 is the upper triangle, every lag > 0 is the full m x m (the harness's own universe)
        pair_windows = n_steps * (m * (m - 1) // 2 + (n_lag_probes - 1) * m * m)
        r["pair_windows"] = pair_windows
        for mode in ("bruteforce", "bf_incremental", "naive_numpy", "naive_python"):
            if mode in r:
                r[mode]["ns_per_pair_window"] = 1e9 * r[mode]["wall"] / pair_windows
        print(f"m={m:5d}: per pair-window (ns): bruteforce {r['bruteforce']['ns_per_pair_window']:8.0f} | "
              f"bf_incremental {r['bf_incremental']['ns_per_pair_window']:7.0f} | naive_numpy {r['naive_numpy']['ns_per_pair_window']:8.0f}"
              + (f" | naive_python {r['naive_python']['ns_per_pair_window']:9.0f}" if "naive_python" in r else "")
              + f"   ({pair_windows:,} pair-windows)", flush=True)
        print(f"m={m:5d}: bruteforce {bfw:8.2f}s | bf_incremental {r['bf_incremental']['wall']:8.2f}s ({bfw / r['bf_incremental']['wall']:.2f}x) | "
              ...
              + f" | counts bf/stomp/numpy{'/py' if 'naive_python' in r else ''}: {r['bruteforce']['correlated']}/{r['bf_incremental']['correlated']}/{r['naive_numpy']['correlated']}"
